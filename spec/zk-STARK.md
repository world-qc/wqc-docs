...
2. **SNARK wrap of $\pi_{\text{Root}}$ (E5b-3 thicken):** Thin toolchain done (E5b-2a–2c; E5b-2d thin KPIs). **E5b-3a/3b/3c Done**; **E5b-3d Partial** (`thick_fri_fs_auth_v0` N=40 + `thick_recagg_fri_fs_auth_v0` N=40 + OOD/AIR↔Auth + `thick_child_verify_var_v0` Depth∈{1..4} digest mux ~11.774M (Phase-2b Auth×2 stripped) folded into `thick_unified_v0` ~398.03M est + leaf FriFsAuth N=1 + multi-chunk Quot zps(ζ) + standalone `thick_child_verify_leaf_auth_v0` ~8.431M + `thick_child_digest_var_v0` Len∈{1..256} ~45.878M). Remaining for wrap gate: Max→87KiB / deeper Auth / payload↔Auth, statement ≡ `verify_root_proof`, then audit + target-L2 gas (`on-chain_settlement_scope.md` §7.2 / §7.6). Thin wrap is not full on-L2 root validity.
